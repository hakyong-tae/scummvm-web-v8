import { assetUrl, GAME, GAME_TITLE, gameText } from '../config'

/** 라이선스·출처 고지 모달. 원본 LICENSE/README 전문은 배포본 안의 게임 데이터 폴더에서 그대로 읽는다. */
export async function openNotice(lang: string = 'ko') {
  const originals = await Promise.all(GAME.licenseFiles.map(f =>
    fetch(assetUrl(`engine/data/games/${GAME.id}/${f}`)).then(r => r.ok ? r.text() : '').catch(() => '')))
  const src = import.meta.env.VITE_SOURCE_URL as string | undefined
  const origin = gameText(lang).origin
  const wrap = document.createElement('div'); wrap.className = 'modal'
  wrap.innerHTML = `
    <div class="modal-box">
      <h2>${GAME_TITLE}</h2>
      ${lang === 'en' ? `<p><b>Original</b> ${origin} This edition does <b>not modify a single byte</b> of the original game data and charges nothing for the game itself.</p>
      ...
      <p><b>Font</b> Neo둥근모 © Eunbin Jeong (Dalgona.) — SIL Open Font License 1.1.</p>` : `<p><b>원작</b> ${origin} 이 배포본은 원본 게임 데이터를 <b>바이트 하나도 수정하지 않았으며</b>, 게임 자체에 요금을 부과하지 않습니다.</p>
      ...
      <details><summary>${lang === 'en' ? 'Original LICENSE.txt / README' : '원본 LICENSE.txt / README 전문'}</summary><pre>${originals.map(escapeHtml).join('\n\n')}</pre></details>
